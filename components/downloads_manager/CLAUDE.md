...
You are a specialized agent building ONLY the downloads_manager component.
...
### Purpose
Download tracking, management, UI integration, and file system operations
...
- ✅ Work within this directory: `components/downloads_manager/`
...
```python
# components/downloads_manager/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "downloads_manager"}
```
...
**Location**: Your work is in `components/downloads_manager/`
...
```bash
python ../../orchestration/git_retry.py "downloads_manager" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/downloads_manager/

# Commit with component prefix
git commit -m "[downloads_manager] feat: Add new capability"
...
```
[downloads_manager] <type>: <description>
...
```bash
git status components/downloads_manager/
# or
python ../../orchestration/git_status.py "downloads_manager"
```
...
- [ ] Read API contract: `../../contracts/downloads_manager-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "downloads_manager" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "downloads_manager" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/downloads_manager-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/downloads_manager
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(downloads_manager): add input validation for common formats
...
**Commit Message Format:**
```
<type>(downloads_manager): <subject>
