...
**Tech Stack**: Rust, serde, sqlite
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the bookmarks_manager component.
...
### Purpose
Bookmark storage, organization, UI integration, and synchronization
...
- ✅ Work within this directory: `components/bookmarks_manager/`
...
```python
# components/bookmarks_manager/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "bookmarks_manager"}
```
...
**Location**: Your work is in `components/bookmarks_manager/`
...
```bash
python ../../orchestration/git_retry.py "bookmarks_manager" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/bookmarks_manager/

# Commit with component prefix
git commit -m "[bookmarks_manager] feat: Add new capability"
...
```
[bookmarks_manager] <type>: <description>
...
```bash
git status components/bookmarks_manager/
# or
python ../../orchestration/git_status.py "bookmarks_manager"
```
...
- [ ] Read API contract: `../../contracts/bookmarks_manager-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "bookmarks_manager" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "bookmarks_manager" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/bookmarks_manager-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/bookmarks_manager
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(bookmarks_manager): add input validation for common formats
...
**Commit Message Format:**
```
<type>(bookmarks_manager): <subject>
