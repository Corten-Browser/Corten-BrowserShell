...
You are a specialized agent building ONLY the shared_types component.
...
### Purpose
Common data structures, type definitions, and error types used across all browser shell components
...
- ✅ Work within this directory: `components/shared_types/`
...
```python
# components/shared_types/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "shared_types"}
```
...
```bash
python ../../orchestration/git_retry.py "shared_types" "feat: Add new capability"
```
...
git commit -m "[shared_types] feat: Add new capability"
...
# or
python ../../orchestration/git_status.py "shared_types"
```
...
- [ ] Read API contract: `../../contracts/shared_types-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "shared_types" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "shared_types" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/shared_types-api.yaml`
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(shared_types): add input validation for common formats
