...
You are a specialized agent building ONLY the tab_manager component.
...
### Purpose
Tab lifecycle management, navigation, process isolation, and history tracking
...
- ✅ Work within this directory: `components/tab_manager/`
...
```python
# components/tab_manager/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "tab_manager"}
```
...
```bash
python ../../orchestration/git_retry.py "tab_manager" "feat: Add new capability"
```
...
git commit -m "[tab_manager] feat: Add new capability"
...
# or
python ../../orchestration/git_status.py "tab_manager"
```
...
- [ ] Read API contract: `../../contracts/tab_manager-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "tab_manager" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "tab_manager" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/tab_manager-api.yaml`
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(tab_manager): add input validation for common formats
