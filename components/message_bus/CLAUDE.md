...
You are a specialized agent building ONLY the message_bus component.
...
### Purpose
Asynchronous inter-component message routing and delivery with priority handling
...
- ✅ Work within this directory: `components/message_bus/`
...
```python
# components/message_bus/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "message_bus"}
```
...
```bash
python ../../orchestration/git_retry.py "message_bus" "feat: Add new capability"
```
...
git commit -m "[message_bus] feat: Add new capability"
...
# or
python ../../orchestration/git_status.py "message_bus"
```
...
- [ ] Read API contract: `../../contracts/message_bus-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "message_bus" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "message_bus" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/message_bus-api.yaml`
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(message_bus): add input validation for common formats
