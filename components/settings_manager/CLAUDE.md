...
**Tech Stack**: Rust, serde (serialization), sqlite (storage)
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the settings_manager component.
...
### Purpose
User settings, preferences management, persistence, and synchronization
...
- ✅ Work within this directory: `components/settings_manager/`
...
```python
# components/settings_manager/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "settings_manager"}
```
...
**Location**: Your work is in `components/settings_manager/`
...
```bash
python ../../orchestration/git_retry.py "settings_manager" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/settings_manager/

# Commit with component prefix
git commit -m "[settings_manager] feat: Add new capability"
...
```
[settings_manager] <type>: <description>
...
```bash
git status components/settings_manager/
# or
python ../../orchestration/git_status.py "settings_manager"
```
...
- [ ] Read API contract: `../../contracts/settings_manager-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "settings_manager" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "settings_manager" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/settings_manager-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/settings_manager
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(settings_manager): add input validation for common formats
