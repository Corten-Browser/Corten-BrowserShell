...
**Tech Stack**: Rust, clap (CLI parsing)
**Current Token Budget**: 0/200,000

You are a specialized agent building ONLY the shell_app component.
...
### Purpose
Application entry point, CLI argument parsing, and browser shell initialization
...
- ✅ Work within this directory: `components/shell_app/`
...
```python
# components/shell_app/tests/integration/test_component.py
...
def check_health():
    ...
    return {"status": "healthy", "component": "shell_app"}
```
...
**Location**: Your work is in `components/shell_app/`
...
```bash
python ../../orchestration/git_retry.py "shell_app" "feat: Add new capability"
```

**Option 2: Direct git with manual retry**
```bash
# Stage only your component files
git add components/shell_app/

# Commit with component prefix
git commit -m "[shell_app] feat: Add new capability"
...
```bash
git status components/shell_app/
# or
python ../../orchestration/git_status.py "shell_app"
```
...
- [ ] Read API contract: `../../contracts/shell_app-api.yaml`
...
6. **Commit** with TDD pattern
   - Use retry wrapper:
   - `python ../../orchestration/git_retry.py "shell_app" "test/feat/refactor: [description]"`
...
python ../../orchestration/git_retry.py "shell_app" "feat: implement user registration
...
### API Contract
- Location: `../../contracts/shell_app-api.yaml`
...
```bash
python orchestration/test_quality_checker.py components/shell_app
```
...
2. **Commit immediately** without asking permission:
   ```bash
   git add .
   git commit -m "feat(shell_app): add input validation for common formats
